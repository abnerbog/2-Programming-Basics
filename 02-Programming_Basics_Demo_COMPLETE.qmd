...
![](images/dplyr functions.png "Dpylr functions")
...
print(filter(rbi, STATE == "ME" & RBI > 0.25))
...
```{r echo=TRUE}
print(arrange(rbi, RBI))

print(arrange(rbi, desc(RBI)))
...
```{r echo=TRUE}
print(select(rbi, STANAME, STATE, RBI))

print(select(rbi, site_no:DRAIN_SQKM))
```
...
```{r echo=TRUE}
print(mutate(rbi, T_RANGE = T_MAX_SITE - T_MIN_SITE))
```
...
```{r echo=TRUE}
print(mutate(rbi, site_no = as.character(site_no)))
```
...
```{r echo=TRUE}
print(summarize(rbi, meanrbi = mean(RBI), maxrbi = max(RBI), minrbi = min(RBI)))
```
...
print(summarize(rbistate, meanrbi = mean(RBI), maxrbi = max(RBI), minrbi = min(RBI)))
```
